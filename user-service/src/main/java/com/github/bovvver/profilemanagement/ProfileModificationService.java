...
class ProfileModificationService {
    ...
    ProfileUpdateResponse updateProfile(final ProfileUpdateRequest request) {
        ProviderProfileEntity profileEntity = providerProfileReadRepository.findByUserId(request.userId())
                .orElseThrow(() -> new UserNotFoundException("Profile not found for userId=" + request.userId()));
